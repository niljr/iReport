...
function autocomplete(input, latInput, lngInput) {
  ...
  dropdown.setComponentRestrictions({
    'country': ['ph']
  });
...
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 10.7;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 122.5;

	_axios2.default.get('/api/reports/near?lat=' + lat + '&lng=' + lng).then(function (res) {
		var places = res.data;
		if (!places.length) {
...
function searchResultsHTML(reports) {
	return reports.map(function (report) {
		return '\n\t\t\t<a href="/report/' + report.slug + '" class="search__result">\n\t\t\t\t<strong>' + report.title + '</strong>\n\t\t\t</a>\n\t\t';
	}).join('');
}
